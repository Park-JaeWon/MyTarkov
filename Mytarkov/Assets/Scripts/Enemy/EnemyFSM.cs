...
public class EnemyFSM : MonoBehaviour
{
    ...
    [SerializeField]
    private float pursuitLimitRange = 10;//추적 범위(이 범위 바깥으로 나가면 Wander상태로 변경)
    ...
    private EnemyState enemyState = EnemyState.None;//현재 적 상태
    private float lastAttackTime = 0;//공격 주기 계산

    private Status status;//이동속도 등의 정보
    private NavMeshAgent navMeshAgent;//이동 제어
    ...
    {
        StopCoroutine(enemyState.ToString());//현재 상태 정지
    ...
    private IEnumerator Pursuit()
    {
        while (true)
        {
            ...
            //이동 속도 설정(배회할 때는 걷는 속도, 추적할 때는 뛰는 속도)
    ...
    private void CalculateDistanceToTargetAndSelectState()
    {
        ...
        else if(distance <= targetRecongnitionRange)
        {
